...
struct Take <: Phase end
struct Put <: Phase end

function funnel(p::Type{Take}, f, inc, args...; csize=0, ctype=Any, kwargs...)
...
function funnel(p::Type{Put}, f, inc, args...; csize=0, ctype=Any, kwargs...)
  outc = Channel{ctype}(csize)
  task = @async while true
...
function stage!(p::Type{P}, f, rc::RemoteChannel, args...; kwargs...) where P <: Phase
  pid = rc.where
  remote_do(pid) do
...
function stage!(p::Type{P}, f, rcs::Vector{RemoteChannel}, args...; kwargs...) where P <: Phase
  map(rcs) do rc
...
function merges!(cs::Container{C}; csize=0, ctype=Any) where C <: Channel
